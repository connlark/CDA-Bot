...
const marketData = require('./coinex-markets.json');
...
const Trader = function(config) {
  ...
  //this.coinex = new CoinEx(this.key, this.secret);
  this.coinex = new CoinEx('BCD13880BF124F15810CC95516B9687F', '175DB51424B342EF827C0F03DEC658AB6A2FF4E78200C904');
};
...
Trader.prototype.getTrades = function(since, callback, descending) {
  ...
  const processResults = (err, data) => {
    ...
    _.each(
      ...
      function(trade) {
        parsedTrades.push({
          tid: trade.id,
          date: moment(trade.create_time).unix(),
  ...
  const fetch = cb => this.coinex.transactions(this.pair);
  this.coinex.transactions(this.pair).then((data) => processResults(null, data))
};
...
Trader.prototype.getPortfolio = function(callback) {
  ...
  const setBalance = (err, data) => {
    ...
    const assetAmount = parseFloat(_.find(data.available, findAsset));

    const findCurrency = item => item.asset === this.currency;
    const currencyAmount = parseFloat(_.find(data.available, findCurrency));
  ...
  const fetch = cb => this.coinex.balance();
...
Trader.prototype.getTicker = function(callback) {
  ...
  const setTicker = (err, data) => {
    ...
    var ticker = {
      ask: parseFloat(data[this.pair].buy_amount),
      bid: parseFloat(data[this.pair].sell_amount),
    };
    console.log(ticker)
  ...
  const handler = cb => this.coinex.tickerAll();
